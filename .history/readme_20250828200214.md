...
Event Bubbling is a process in the DOM where an event first happens on the innermost element (the one you directly interact with), and then it automatically travels upward through its parent, grandparent, and so on until it reaches the top (document), unless stopped.
